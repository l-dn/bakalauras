...
x_n = [mpmath.mpf('0') for i in range(0, n_max+4)]
y_n = [mpmath.mpf('0') for i in range(0, n_max+4)]
s_n = [mpmath.mpf('0') for i in range(0, n_max+4)]
...
if (s_n[0]==0):
    print("make sure: s0 > 0, else use different code");
    sys.exit()

print()

# Initialize high enough elements
alpha_n=[mpmath.mpf('0') for i in range(0, n_max+4)]
beta_n=[mpmath.mpf('0') for i in range(0, n_max+4)]
gamma_n=[mpmath.mpf('0') for i in range(0, n_max+4)]
delta_n=[mpmath.mpf('0') for i in range(0, n_max+4)]

alpha_n[0]=mpmath.mpf('1')
alpha_n[3]=-1 / s_n[0]

beta_n[1]=mpmath.mpf('1')
beta_n[3]=-1*(S_U[2]+(1-X_U[2])*y_n[0]+(1-X_U[1])*y_n[1])/s_n[0]

gamma_n[2]=mpmath.mpf('1')
gamma_n[3]=-1*(S_U[1]+(1-X_U[1])*y_n[0])/s_n[0]

delta_n[3]=1/s_n[0]

for i in range(4, n_max+4):
    vid=mpmath.mpf('0')
    for k in range(1, i):
        vid=vid+s_n[i-k]*alpha_n[k]

    alpha_n[i]=(1/s_n[0])*(alpha_n[i-4]-vid)

for i in range(4, n_max+4):
    vid=mpmath.mpf('0')
    for k in range(1, i):
        vid=vid+s_n[i-k]*beta_n[k]

    beta_n[i]=(1/s_n[0])*(beta_n[i-4]-vid+x_n[i-1]*y_n[0]+x_n[i-2]*y_n[1])

for i in range(4, n_max+4):
    vid=mpmath.mpf('0')
    for k in range(1, i):
        vid=vid+s_n[i-k]*gamma_n[k]

    gamma_n[i]=(1/s_n[0])*(gamma_n[i-4]-vid+x_n[i-2]*y_n[0])

for i in range(4, n_max+4):
    vid=mpmath.mpf('0')
    for k in range(1, i):
        vid=vid+s_n[i-k]*delta_n[k]

    delta_n[i]=(1/s_n[0])*(delta_n[i-4]-vid)

# High enough elements
alpha_1=alpha_n[n_max+1]-alpha_n[n_max]
alpha_2=alpha_n[n_max+2]-alpha_n[n_max]
alpha_3=alpha_n[n_max+3]-alpha_n[n_max]

beta_1=beta_n[n_max+1]-beta_n[n_max]
beta_2=beta_n[n_max+2]-beta_n[n_max]
beta_3=beta_n[n_max+3]-beta_n[n_max]

gamma_1=gamma_n[n_max+1]-gamma_n[n_max]
gamma_2=gamma_n[n_max+2]-gamma_n[n_max]
gamma_3=gamma_n[n_max+3]-gamma_n[n_max]

delta_1=delta_n[n_max+1]-delta_n[n_max]
delta_2=delta_n[n_max+2]-delta_n[n_max]
delta_3=delta_n[n_max+3]-delta_n[n_max]

# Initialize matrixes
A=mpmath.matrix([[alpha_1, beta_1, gamma_1],
...
Phi_ = ((-Phi_U[0])-((S_U[2])+(1-(X_U[2]))*(y_n[0])+(1-(X_U[1]))*(y_n[1]))*(Phi_U[1]) \
    -((S_U[1])+(1-(X_U[1]))*(y_n[0]))*(Phi_U[2])+(D))/(s_n[0])

# Count Phi_U(3)
Phi_U[3] = Phi_[0]

# Remaining Phi_U
for i in range(4, u_max+1):
    vid=mpmath.mpf('0')
    for k in range(1, i):
        vid=vid+s_n[i-k]*Phi_U[k]
    Phi_U[i]=(1/s_n[0])*(Phi_U[i-4]+(x_n[i-1]*y_n[0]+x_n[i-2]*y_n[1])*Phi_U[1] \
        +x_n[i-2]*y_n[0]*Phi_U[2]-vid)
...
with open("ultimate_time_s0.txt", "w") as f:
